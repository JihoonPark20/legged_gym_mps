...
def parse_arguments(description="Isaac Gym Example", headless=False, no_graphics=False, custom_parameters=[]):
    ...
    parser.add_argument('--sim_device', type=str, default="mps", help='Physics Device in PyTorch-like syntax')
    ...
    assert (pipeline == 'cpu' or pipeline in ('gpu', 'mps')), f"Invalid pipeline '{args.pipeline}'. Should be either cpu or gpu."
    args.use_gpu_pipeline = (pipeline in ('gpu', 'mps'))

    if args.sim_device_type != 'mps' and args.flex:
        print("Can't use Flex with CPU. Changing sim device to 'mps'")
        args.sim_device = 'mps'
        args.sim_device_type, args.compute_device_id = parse_device_str(args.sim_device)

    if (args.sim_device_type != 'mps' and pipeline == 'gpu'):
